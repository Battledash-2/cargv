		...
		for (let i = 0; i<args.length+1; i++) {
			...
			if (helpCommand && doesMatch(clo, 'help')) {
				...
				if (next == undefined) {
					...
					list.iterate(opt=>{
						writeln('\t'+clrs.blue(opt.name)+clrs.hex('#f9f9f9', ': ')+clrs.yellow(opt.description) + ' ' + clrs.hex('#444', '(')+clrs.magenta(typeof opt.type === 'object' ? opt.type.join(clrs.hex('#555', '>')) : opt.type)+clrs.hex('#444', ')'));
					});
				} else {
					let option;
					list.iterate(opt=>{
					...
					writeln('\t'+clrs.italic('Type: ')+clrs.magenta(type.toLowerCase()));
					...
					if (option.usage != null) {
						...
						for (let i = 0; i<option.arguments; i++) {
							let type = (typeof option.type === 'object') ? option.type[i]||option.type[0] : option.type;
							usage += ' ';
			...
			list.iterate(opt => {
				...
				if (type.toLowerCase() === 'none') {
					...
					for (let p = 0; p<opt.arguments; p++) {
						let type = (typeof opt.type === 'object') ? opt.type[p]||option.type[0] : opt.type;
						let next = args[i++];
